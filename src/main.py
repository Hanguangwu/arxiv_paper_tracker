...
# 配置 OpenAI（OpenAI SDK >= 1.0 新接口）
# 兼容 DeepSeek 等任何 OpenAI 兼容端点，需设置 LLM_BASE_URL
llm_client = OpenAI(
    api_key=LLM_API_KEY,
    base_url=LLM_BASE_URL,
) if LLM_API_KEY else None

# 构建可复用的 arXiv API 客户端（arxiv>=4 使用 Client().results(search)）
...
def download_paper(paper, output_dir):
    ...
    if not paper.pdf_url:
        logger.error(f"论文无可下载的PDF链接: {paper.title}")
        return None
...
def analyze_paper_with_llm(pdf_path, paper):
    """使用LLM API分析论文（OpenAI SDK 2.x/1.x 接口）"""
    if not llm_client:
        logger.error("LLM_API_KEY 未配置，无法分析论文")
        return "**论文分析出错**: LLM_API_KEY 未配置"
...
def send_email(content):
    ...
    try:
        ...
        # 端口 465 使用隐式 SSL，其余使用 STARTTLS（QQ 邮箱两者均可，但 SMTP_USERNAME 必须是完整邮箱账号）
        if SMTP_PORT == 465:
...
def main():
    ...
    for i, paper in enumerate(papers, 1):
        ...
        if pdf_path:
            ...
            # 分析论文
            analysis = analyze_paper_with_llm(pdf_path, paper)
