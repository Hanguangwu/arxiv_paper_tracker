# ArXiv Paper Analysis Report - 2026-08-09
...
Learning When to Trust via Selective Context Preference Optimization proposes a new benchmark MIST and a training method SCOPE that jointly optimizes models to selectively trust context, reducing the rate at which misleading signals flip correct answers while maintaining performance on clean or irrelevant contexts.
...
Automated heart‑failure feature engineering remains a bottleneck; the authors present Nimblemind Multi‑Agent System (nMAS), a rule‑based, evidence‑linked pipeline that generates structured and rubric‑scored aggregated features from fragmented EHR data, achieving a 0.068 AUROC gain for HFrEF and 0.040 for HFpEF phenotyping on a 500‑record cohort, with 81.5 % rubric compliance and auditability via a restricted LLM.
...
Investigating Artificial Intelligence Digital Sovereignty in Mobile Shopping Apps: A Case Study of Nigeria reveals that AI is pervasive in Nigerian e‑commerce apps but transparency about its use is limited, thereby compromising users’ digital sovereignty; the study combines forensic app analysis with contextual document review to assess AI features and disclosure practices, and contextualizes findings within Nigeria’s socio‑economic landscape to highlight uneven AI awareness and growing dependence on digital platforms.
...
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06363v1

A concise summary: The paper presents an agnostic PAC learning algorithm for binary classification that achieves the statistically optimal risk bound up to universal constants, matching known lower bounds for any fixed target risk \(L^*\) and VC dimension \(d\).
...
AV-AIVAT combines variance‑reducing AIVAT corrections with anytime‑valid confidence sequences to enable early, auditable stopping of agent evaluations in imperfect‑information games, achieving up to 74× fewer hands needed compared to raw outcomes while providing finite‑sample guarantees.
...
**Categories**: cs.AI
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06361v1

Real‑world video benchmarks conflate event count, frequency, duration, and visual complexity, obscuring failure modes; this paper introduces trace‑grounded parametric profiling to isolate these factors, evaluates large language models on controlled bouncing‑ball, blinking, and state‑transition tasks, and finds that models reliably count persistent state changes only at low frequencies, while transient events and high‑frequency regimes yield catastrophic failures, even when sampling rates or prompting are increased.
...
Resourced Authority proposes a formal mechanism‑design framework for continuous participatory governance of deployed AI agents, where human stakeholders sequentially contribute in a distinct governance currency that is converted into a binary authorization via a two‑threshold gate with hysteresis; authorized compute budgets are then released as signed hardware compute licenses, ensuring self‑enforcement and aligning with the Safe AI paradigm that treats compute as a governance lever.
...
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06352v1

Training terminal agents demands tasks that are both executable and appropriately challenging; CalibForge addresses this by autonomously synthesizing terminal tasks and refining them through adversarial solver calibration, leveraging verified solver behavior to adjust task difficulty. It introduces two calibration strategies—multi‑solver calibration, which seeks disagreement across a heterogeneous solver pool, and contrastive solver calibration, which enforces a strong‑pass/weak‑fail relationship—to define a solver‑relative learnable zone. Applying CalibForge to generate 5,431 calibrated tasks, the authors demonstrate significant performance gains on Terminal‑Bench 2.0, SWE‑bench Pro, and Doc2Repo, validating the effectiveness of solver‑relative learnability for creating transferable training data.
...
DetoxAI’s evaluation shortcomings are highlighted through bias detection and concept unlearning, followed by a human‑grounded image‑classification explanation study, and an exploration of adapting counterfactual explanations to concept drift in evolving data streams.
...
TrajDebug introduces an error‑lifecycle tracing framework that tackles cascading errors in long‑horizon LLM‑based agent trajectories by compressing multi‑granularity history and using evidence‑based error identification; it further attributes each error’s resolution status and terminal impact. The authors build TrajErrBench, a 486‑sample benchmark of manually annotated failed trajectories from Tau2Bench and SWE‑Bench Pro, covering realistic tool‑use and coding scenarios. Experiments across diverse agent benchmarks demonstrate that TrajDebug outperforms existing baselines in locating the earliest critical error step, and its diagnoses provide actionable feedback for improving downstream agent success. The paper also discusses the release of code and data to foster further research.
...
Scalable estimation of VARMA models introduces a computationally efficient framework that decouples optimization cost from series length, enabling practical estimation of high-dimensional VARMA processes by using a partial-autocorrelation reparametrization, Gaussian priors with distinct diagonal/off-diagonal scales, and Parseval-based loss functions that rely only on fixed-size sufficient statistics; the resulting regularized least-squares and covariance-marginalized MAP estimators recover the true infinite-autoregressive representation at near-parametric rates, extend naturally to seasonal, exogenous, and rolling-window settings, and empirically outperform traditional VAR, Bayesian-VAR, component-wise ARMA, and sparse-VARMA baselines on diverse datasets.
...
Optimal Rates for Learning with Monotone Adversaries: a monotone adversary can add correctly labeled examples to an i.i.d. sample, breaking exchangeability; the paper proves that for VC dimension ≥ 2 the minimax expected error is Θ((d/n) log(n/d)), showing the extra logarithmic factor is unavoidable, and that the same holds for Littlestone dimension, while for d = 1 the rate reverts to Θ(1/n).
...
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06331v1

TYTAN automatically builds analytic semantic schemas for relational databases by combining symbolic database analysis with LLM-driven inference and interactive clarification, achieving perfect coverage and retrieval accuracy on diverse test sets.
...
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06329v1

Benchmarking the Benchmarks: Evaluating Benchmarks for Conversational Agents introduces a reference‑free evaluation framework that employs large language model judges to assess benchmark consistency, complexity, and policy coverage, providing actionable diagnostics and validating the approach against human annotations across synthetic and curated datasets.
...
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06311v1

White matter hyperintensities (WMH) on FLAIR MRI are often missed due to low through‑plane resolution; this study evaluates whether super‑resolution (SR) techniques preserve or distort small lesions by comparing SR‑upsampled thick‑slice scans to high‑resolution ground truth, finding that SR mainly erases small lesions rather than hallucinating them, with ECLARE performing best.
...
Unlocking generative reward models for reinforcement learning, the paper introduces Ranking-based Reward Construction (RRC) that transforms comparative preference rankings into scalar rewards, enabling effective RL training; it proposes self‑competitive and anchor‑guided ranking strategies, and demonstrates consistent performance gains on open‑ended chat and reasoning benchmarks.
...
HarnessOpt-Bench introduces a benchmark for evaluating large language models as automated harness optimizers, where an LLM edits a coding harness to improve agent performance under a fixed evaluation budget, and demonstrates that optimizer models can outperform shared coding harnesses, with native harnesses not consistently superior, revealing significant task‑dependent gains and a large room for improvement.
...
**Categories**: cs.AI
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06300v1

This study extends Concept Activation Vector (CAV) bias analysis to transformer-based L2 speaking graders, comparing a text-only BERT model and a multimodal Whisper-based system, and evaluates whether sparse autoencoders can improve concept recoverability and influence detection, finding that recoverability and sensitivity are architecture-dependent and that SAEs enhance linear separability but reduce activation-space sensitivity, underscoring the need to separate concept encoding from influence when auditing bias.
...
On-policy self-distillation can be achieved without external supervision by leveraging a model’s own generations, using majority-vote pseudo-solutions and self-consistency thresholds to guide corrective distillation, thereby matching or surpassing supervised methods across diverse benchmarks.
...
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06288v1
...
**Categories**: cs.AI
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06270v1
...
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06265v1
...
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06262v1
...
**Categories**: cs.LG
**Published**: 2026-08-06
**Link**: http://arxiv.org/abs/2608.06259v1
...
- data (score: 0.0781)
- based (score: 0.0743)
- framework (score: 0.0662)
- agent (score: 0.0623)
- models (score: 0.0600)
- evaluation (score: 0.0562)
- benchmarks (score: 0.0547)
- error (score: 0.0542)
- evidence (score: 0.0541)
- adaptive (score: 0.0530)
- analysis (score: 0.0521)
- agents (score: 0.0515)
- non (score: 0.0514)
- learning (score: 0.0512)
- optimal (score: 0.0511)
- space (score: 0.0504)
- agentic (score: 0.0490)
- sample (score: 0.0482)
- problem (score: 0.0449)

## Analysis Summary
Trend analysis generation failed（Error code: 429 - {'error': {'message': 'Rate limit exceeded: free-models-per-day. Add 10 credits to unlock 1000 free model requests per day', 'code': 429, 'metadata': {'headers': {'X-RateLimit-Limit': '50', 'X-RateLimit-Remaining': '0', 'X-RateLimit-Reset': '1786320000000'}, 'limit_source': 'openrouter_free_tier_daily', 'remedy_hint': 'Wait for the daily reset (see X-RateLimit-Reset), or purchase credits to raise your free-model daily limit.', 'provider_name': None}}, 'user_id': 'user_37jq2eCfuQPpOGiOzgGvXbzM32b'}）
